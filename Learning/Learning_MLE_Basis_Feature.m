function model = Learning_MLE_Basis_Feature( Seqs, model, alg )
                                                        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Learning SentHawkes via maximum likelihood estimation
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% the MLE with sparse regularizer (MLES)
if alg.Sparse
    US = zeros(size(Aest));
    ZS = Aest;
end

% MLE with a sparse-group-lasso regularizer (SGL)
if alg.GroupSparse
    UG = zeros(size(Aest));
    ZG = Aest;
end
...
for o = 1:alg.outer
            ...
            if ~isempty(Seqs(c).Time)
                ...
                for i = 1:Nc
                    ...
                    if i>2
                        ...
                        gij = Kernel(dt, model);
                        if i>10
                            m_i = abs(Sentiment(i-1) - Sentiment(i-10:i-2));
                        else
                            m_i = abs(Sentiment(i-1) - Sentiment(1:i-2));
                        end
                        m_i = exp(-mean(m_i));
                        auiuj = Aest(uj, :, ui);
                        pij = auiuj .* gij * m_i;
    ...
    % store loglikelihood
    if alg.storeLL
        Loglike = Loglike_Basis_Feature(Seqs, model, alg );
        model.LL(o) = Loglike;
    end
    % calculate error
    if alg.storeErr
