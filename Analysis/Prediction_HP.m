function [lambda_current, count_expect, events] = Prediction_HP(t_start, t_end, ...
...
for n = 1:options.NumTest
    ...
    while t<t_end && size(Hist, 2)<options.Nmax
        ...
        if t+s>t_end || U>mts/mt
            ...
            t = t+s;
            if para.Sentiment
                Hist_test = para.History_test;
                iii = find(Hist_test(1,:)<=t & Hist_test(2,:)==d);
                if ~isempty(iii)
                    Hist=[Hist,[t;index(1);Hist_test(3,iii(end))]];
                else
                    Hist=[Hist,[t;index(1);Hist(3,end)]];
                end
            else
                Hist=[Hist,[t;index(1)]];
            end
